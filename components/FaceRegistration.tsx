...
import { Camera, CheckCircle2, UserPlus, X, Loader2 } from 'lucide-react';
...
export default function FaceRegistration({ userId, userName, onClose }: FaceRegistrationProps) {
    ...
    try {
      ...
      try {
        await fetch('/api/face-enroll', {
         ...
         console.log("Mock supabase call failed, but local storage succeeded");
      }
      ...
      <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden border border-slate-100 flex flex-col">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50">
          <div className="flex items-center gap-3">
             ...
             <div>
               <h2 className="text-xl font-bold text-slate-800">Face Registration</h2>
        ...
        <div className="p-6 flex flex-col items-center gap-6">
          <div className="relative w-64 h-64 bg-slate-100 rounded-full overflow-hidden border-4 border-brand-200 shadow-inner flex items-center justify-center">
            ...
            />
            {status === "success" && (
              <div className="absolute inset-0 bg-emerald-500/20 backdrop-blur-sm flex items-center justify-center text-emerald-600">
